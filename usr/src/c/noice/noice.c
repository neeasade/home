...
redraw(char *path)
{
	...
	/* No text wrapping in getcwd(tmp, sizeof(tmp))cwd line */
	ncols = COLS;
	if (ncols > PATH_MAX)
		ncols = PATH_MAX;
...
nochange:
		...
		case SEL_CD:
			...
			goto begin;
		case SEL_CDHOME:
            strcpy(tmp, home); goto cd;
        case SEL_CDHOMETMP:
            strcat(strcpy(tmp, home), "/tmp"); goto cd;
        case SEL_CDHOMESRC:
            strcat(strcpy(tmp, home), "/usr/src"); goto cd;
        case SEL_CDHOMEDL:
            strcpy(tmp, getenv("XDG_DOWNLOAD_DIR")); goto cd;
        case SEL_CDHOMERPS:
            strcat(strcpy(tmp, home), "/usr/repos"); goto cd;
        case SEL_CDHOMEWLL:
            strcat(strcpy(tmp, home), "/var/pictures/wallpapers"); goto cd;
        case SEL_CDHOMEETC:
            strcat(strcpy(tmp, home), "/etc"); goto cd;
        case SEL_CDHOMECHE:
            strcpy(tmp, getenv("XDG_CACHE_HOME")); goto cd;
        case SEL_CDHOMEUSR:
            strcat(strcpy(tmp, home), "/usr"); goto cd;
		case SEL_TOGGLEDOT:
			showhidden ^= 1;
